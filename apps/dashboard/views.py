...
def index(request):
    ...
    try:
        ...
        ctx['ca_mois']        = Facture.objects.filter(date_facture__range=[debut_mois, fin_mois], statut__in=['en_attente','partielle','payee']).aggregate(t=Sum('total_ttc'))['t'] or 0
        ctx['montant_encaisse']= Facture.objects.filter(date_facture__range=[debut_mois, fin_mois]).aggregate(t=Sum('montant_paye'))['t'] or 0
        ...
        logger.exception(f"Erreur dans dashboard index (factures): {e}")
    try:
        ...
        logger.exception(f"Erreur dans dashboard index (clients): {e}")
    try:
        ...
        logger.exception(f"Erreur dans dashboard index (produits): {e}")
    try:
        ...
        logger.exception(f"Erreur dans dashboard index (services): {e}")
    try:
        from apps.locations.models import Location
        ctx['locations_retard'] = Location.objects.filter(statut='en_cours', date_fin_prevue__lt=timezone.now()).count()
    except Exception as e:
        logger.exception(f"Erreur dans dashboard index (locations): {e}")
